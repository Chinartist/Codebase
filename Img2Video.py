...
def img2video_ffmpeg(img_dir,out_dir,fps):
    ...
    os.system(f"ffmpeg -framerate {fps} -i {os.path.join(img_dir, '%*.jpg')} -c:v libx264 -pix_fmt yuv420p {out_file}")

    print(f"Video saved at {out_file}")
    
if __name__ == "__main__":
    # Example usage
    img_dir = '/nvme0/public_data/Occupancy/proj/Generation/cosmos-transfer1/dataset/down'  # Replace with your image directory
    out_dir = '/nvme0/public_data/Occupancy/proj/Generation/cosmos-transfer1/dataset/'  # Replace with your output directory
    img2video_ffmpeg(img_dir, out_dir,30)